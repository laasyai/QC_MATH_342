...
No because the RMSE on the select is not less than the RMSE on the test set.

Create the final model object `g_final`.

```{r}
mod_star = lm(model_formulas[m_star], diamonds_cleaned)
g_final = function(mod_star, x_star){
...
repeat {
  ...
  for (j_try in 1 : p_plus_one){
    ...
    p_hat = 1/(1 + exp(-Xmm_sub %*% coef_mod))
    all_briers[j_try] = -sum((y_train - p_hat)^2) 
  }
  j_star = which.min(all_briers)
  ...
  mod = glm(y_train ~ 0 + Xmm_sub, family = "binomial")
  y_hat_select = Xfull_select[, included_features_by_iter, drop = FALSE] %*% mod$coefficients
