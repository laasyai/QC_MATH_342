...
bills_and_payments_and_discounts %<>%
  group_by(id) %>%
  slice(1) #if a bill is in multiple rows, you end up dropping the rest with "slice"
...
#do customer id's into factor for common customers otherwise "other"
#convert discount id to factor if number of discount ids is > 10. Drop all other columns from discount table
bills_and_payments_and_discounts = bills_and_payments_and_discounts %>%
