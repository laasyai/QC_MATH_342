...
#prediction: a_parallel = 1 2 3 4 a_perpendicular = 0 0 0 0
orthogonal_projection(c(1, 2, 3, 4), c(0, 2, 0, -1))
#prediction: a_parallel = 0 0 0 0 a_perpendicular = 1 2 3 4
...
Write a for loop to each time bind a new column of 100 standard iid normals to the matrix X and find the R^2 each time until the number of columns is 100. Create a vector to save all R^2's. What happened?? The values approach 1.
...
rank is n but X^TX is (n+1)x(n+1)
...
yes because there is double counting since it projects onto the column space of X but it's not an orthogonal projection.
...
Yes, it fails because the basis chosen is not unique.
...
Yes. In sample line drops where the oos line hits the lowest point around p=14 so thats the number of useful features.
